...
// client shows the progressbar
if(hasInterface) then {
	diag_log format ["interface trigger"];
	// lazy load functions if needed for clients
	[] call compileFinal preprocessFileLineNumbers "dyncap\dyncap_fn.sqf";

	// needed for UI interaction
	disableSerialization;

	// give the servers trigger time to do it's thing not sure if this is needed just for safety
	sleep 1;
